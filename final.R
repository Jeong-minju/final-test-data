...
GNI2014_Asia <- mutate(
  GNI2014_Asia,
  GNI_capita = GNI2014_Asia$GNI_capita,
  GNI = GNI2014_Asia$GNI)
View(GNI2014_Asia)
...
library(treemap)
data(GNI2014)
register_google(key='AIzaSyAI-tO-LMNkLVW80R-SzFxotolk5m-OAF0')  # Register your google key.


ds <- subset(GNI2014,continents="North America")
...
cat("The three main sections are header,text and code chunks")


main_sections <- c("text","header","code chunks")

# 2. In an R Markdown, how will commonly we execute R code inline with the rest of text?
# 'r your_R_code', a single backtick(')
cat("by adding the letter r and a space immediately after the first backtick")
...
https://jeong-minju.github.io/Movie_Review/
